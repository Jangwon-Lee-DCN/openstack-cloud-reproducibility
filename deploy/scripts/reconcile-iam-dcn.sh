...
if idp_json=$(curl -sf -H "X-Auth-Token: ${OS_TOKEN}" "$idp_url" 2>/dev/null); then
  ...
  curl -sf -X PATCH -H "X-Auth-Token: ${OS_TOKEN}" -H 'Content-Type: application/json' \
    -d '{"identity_provider":{"enabled":true,"remote_ids":["https://cloud.dcn.ssu.ac.kr/horizon/auth/idp/realms/dcn"]}}' \
    "$idp_url" >/dev/null
else
  curl -sf -X PUT -H "X-Auth-Token: ${OS_TOKEN}" -H 'Content-Type: application/json' \
    -d '{"identity_provider":{"enabled":true,"remote_ids":["https://cloud.dcn.ssu.ac.kr/horizon/auth/idp/realms/dcn"],"domain_id":"'"${DOMAIN_ID}"'"}}' \
    "$idp_url" >/dev/null
fi
