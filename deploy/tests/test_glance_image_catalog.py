...
class CatalogTest(unittest.TestCase):
    ...
    def test_rejects_hidden_capi_image(self):
        data = self.catalog(True, "capi")
        with tempfile.NamedTemporaryFile("w") as stream:
            import yaml
            yaml.safe_dump(data, stream); stream.flush()
            with self.assertRaisesRegex(ValueError, "CAPI image must be visible"):
                MOD.load_catalog(stream.name)

    def test_rejects_visible_amphora_image(self):
        data = self.catalog(False, "amphora")
        with tempfile.NamedTemporaryFile("w") as stream:
            import yaml
            yaml.safe_dump(data, stream); stream.flush()
            with self.assertRaisesRegex(ValueError, "Amphora image must be hidden"):
                MOD.load_catalog(stream.name)
    ...
    def test_apply_sets_metadata_visible_and_protection(self):
        data = self.catalog(False, "capi")
        data["images"][0]["class"] = "service"
        responses = [json.dumps([{"ID":"1","Name":"ubuntu","Checksum":"a"}]), "[]",
                     json.dumps({"id":"1","visibility":"public","protected":False,
                                 "properties":{"os_hidden":True},"tags":[]})]
        with mock.patch.object(MOD, "command", side_effect=responses + [""] * 4) as run:
            ...
            self.assertTrue(any("--unhidden" in call for call in calls))
