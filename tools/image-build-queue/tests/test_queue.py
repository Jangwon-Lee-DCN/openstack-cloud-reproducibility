...
class QueueTests(unittest.TestCase):
    ...
    def test_gpu_profiles_are_version_named_and_serialized(self):
        ...
        self.assertIn("nvidia-container-toolkit containerd python3-venv fio", builder)
        self.assertIn("command -v ctr", builder)
        self.assertIn("command -v nvidia-container-runtime", builder)
    ...
    def test_service_supplies_a_build_capable_python(self):
        ...
        self.assertIn('"$root/dcn_image_build.py" queue --require-no-running', installer)
        self.assertNotIn("/usr/local/bin/dcn-image-build queue --require-no-running", installer)
        ...
        self.assertIn('start --group "$group"', groups)
        self.assertNotIn('group start "$group"', groups)
    ...
    def test_queue_view_defaults_to_active_requests(self):
        ...
        with tempfile.TemporaryDirectory() as temporary, \
            ...
            with redirect_stdout(output):
                ...
                self.assertEqual(queue.queue_view(require_no_running=True), 1)
    ...
    def test_queued_work_does_not_block_safe_daemon_update(self):
        request = {
            "task_id": 2, "group": "glance-images",
            "component": "ubuntu-24.04-cuda-12.9",
            "request_id": "queued-request", "status": "queued",
        }
        with tempfile.TemporaryDirectory() as temporary, \
                mock.patch.object(queue, "STATE", Path(temporary)), \
                mock.patch.object(queue, "effective_status", return_value="queued"):
            request_dir = Path(temporary) / "requests"
            request_dir.mkdir()
            (request_dir / "request.json").write_text(json.dumps(request))
            with redirect_stdout(StringIO()):
                self.assertEqual(queue.queue_view(require_no_running=True), 0)
