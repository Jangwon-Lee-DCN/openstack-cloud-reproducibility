...
def setup():
    ...
    _, resource = call(os.environ["GOVERNANCE_GNOCCHI_URL"] + "/v1/resource/generic", token,
        method="POST", body={"id": resource_id, "project_id": project_id,
                             ...
                             "metrics": {"governance.acceptance": {"archive_policy_name": "low"}}})
    ...
    while time.monotonic() < deadline:
        query = urlencode({"response_format": "object", "groupby": "type",
                           "filters": f"project_id:{project_id}", "begin": begin, "end": end})
        try:
            _, summary = call(os.environ["GOVERNANCE_CLOUDKITTY_URL"] + f"/v2/summary?{query}", token)
            cloudkitty_total = summary.get("total", 0)
        except HTTPError as exc:
...
def verify():
    ...
    print(json.dumps({"duplicate_reprocess": "stable", "ledger_count": count,
...
if __name__ == "__main__":
    ...
    {"setup": setup, "verify": verify, "cleanup": cleanup}.get(action, lambda: (_ for _ in ()).throw(
        RuntimeError("set GOVERNANCE_FINOPS_ACCEPTANCE=setup|verify|cleanup")))()
