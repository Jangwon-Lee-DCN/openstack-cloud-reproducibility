...
virt-customize -a "$image" --network --memsize 4096 --smp 8 \
  ...
  --run-command "$guest_network; curl -fsSLo /tmp/cuda-keyring.deb https://developer.download.nvidia.com/compute/cuda/repos/$repo/x86_64/cuda-keyring_1.1-1_all.deb; dpkg -i /tmp/cuda-keyring.deb; apt-get update; DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends '$driver_package' '$cuda_package' $cudnn_packages 'libnccl2=$nccl_version' 'libnccl-dev=$nccl_version' nvidia-container-toolkit containerd python3-venv fio; for spec in '$driver_package' '$cuda_package' $cudnn_packages 'libnccl2=$nccl_version' 'libnccl-dev=$nccl_version'; do package=\${spec%%=*}; expected=\${spec#*=}; actual=\$(dpkg-query -W -f='\${Version}' \"\$package\"); test \"\$actual\" = \"\$expected\" || { echo \"version mismatch: \$package expected \$expected actual \$actual\" >&2; exit 1; }; done; dpkg-query -W containerd >/dev/null; command -v ctr >/dev/null; command -v nvidia-container-runtime >/dev/null" \
