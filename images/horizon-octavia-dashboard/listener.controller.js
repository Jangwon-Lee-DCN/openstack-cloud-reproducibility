...
(function () {
  ...
  function ListenerDetailsController($scope, gettext) {
    ...
    // availableProtocols is filtered by the load balancer's own type
    // (Octavia provider), computed from model.providerCapabilities
    // (see model.service.js) -- a listener protocol this load
    // balancer's own provider can't serve would otherwise fail with a
    // real Octavia 501 only after submitting the whole wizard.
    // model.spec.loadbalancer.provider is set on the earlier
    // loadbalancer step (create) or comes from the existing object
    // (edit), so this recomputes whenever it's known rather than
    // assuming step ordering.
    ctrl.availableProtocols = $scope.model.listenerProtocols;
    $scope.$watch('model.spec.loadbalancer.provider', function(provider) {
      var capabilities = $scope.model.providerCapabilities[provider];
      ctrl.availableProtocols = capabilities ?
        capabilities.listenerProtocols : $scope.model.listenerProtocols;
      // If the currently-selected protocol is no longer valid for this
      // provider (e.g. the user changed the load balancer type after
      // already picking a protocol), clear it rather than silently
      // submitting an unsupported combination.
      if (
        $scope.model.spec.listener.protocol &&
        ctrl.availableProtocols.indexOf($scope.model.spec.listener.protocol) === -1
      ) {
        $scope.model.spec.listener.protocol = null;
      }
    });
