...
(function() {
  ...
  function LoadBalancerDetailsController(patterns, gettext, $scope) {
    ...
    // Load balancer type (Octavia provider) options -- see
    // model.providerCapabilities for what each one actually supports.
    // Deliberately presented as "L4 only" / "L4 + L7" rather than the
    // raw provider name: the provider itself is an implementation
    // detail this wizard would otherwise be the only place asking an
    // end user to understand.
    ctrl.providerOptions = Object.keys($scope.model.providerCapabilities).map(
      function(provider) {
        return {
          value: provider,
          label: $scope.model.providerCapabilities[provider].label
        };
      }
    );

    ctrl.setProvider = function(provider) {
      $scope.model.spec.loadbalancer.provider = provider;
    };

    // Default to the lighter-weight L4-only type on create; never
    // touch it on edit (Octavia providers are immutable after
    // creation, matching model.context.id disabling the whole control
    // in loadbalancer.html).
    if (!$scope.model.context.id && !$scope.model.spec.loadbalancer.provider) {
      $scope.model.spec.loadbalancer.provider = 'ovn';
    }
