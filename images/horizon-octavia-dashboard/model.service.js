...
(function () {
  ...
  function workflowModel(
    ...
    var model = {
      ...
      // SOURCE_IP_PORT added -- this cluster's "ovn" provider's own
      // native 5-tuple affinity algorithm was previously never
      // selectable in this wizard at all (see providerCapabilities
      // below for why that mattered).
      methods: ['LEAST_CONNECTIONS', 'ROUND_ROBIN', 'SOURCE_IP', 'SOURCE_IP_PORT'],
      // providerCapabilities: real, confirmed per-provider limits on
      // this deployment (not a generic Octavia API constraint -- other
      // providers/deployments may differ). "ovn" cannot do L7 at all
      // (a permanent architectural limitation, not a temporary gap)
      // and only supports its own SOURCE_IP_PORT/SOURCE_IP algorithms;
      // "amphora" (a real HAProxy VM) supports the full set. Used by
      // the loadbalancer/listener/pool steps to keep the wizard from
      // ever offering a combination that would fail with a real
      // Octavia 501 at creation time.
      providerCapabilities: {
        ovn: {
          label: gettext('L4 only (OVN -- lightweight, no VM, lowest latency)'),
          listenerProtocols: ['TCP', 'UDP', 'SCTP'],
          methods: ['SOURCE_IP_PORT', 'SOURCE_IP']
        },
        amphora: {
          label: gettext('L4 + L7 (Amphora -- a managed HAProxy VM, supports HTTP/HTTPS)'),
          listenerProtocols: ['HTTP', 'TCP', 'TERMINATED_HTTPS', 'HTTPS', 'UDP', 'SCTP'],
          methods: ['LEAST_CONNECTIONS', 'ROUND_ROBIN', 'SOURCE_IP', 'SOURCE_IP_PORT']
        }
      },
    ...
    function initialize(resource, id, loadBalancerId, parentResourceId) {
      ...
      model.spec = {
        ...
        loadbalancer: {
          ...
          admin_state_up: true,
          // provider is immutable in Octavia once the load balancer is
          // created, exactly like every other field this step disables
          // via `model.context.id` -- defaulted in
          // LoadBalancerDetailsController on create, left untouched
          // (and its own UI hidden) on edit.
          provider: null
        },
        listener: {
          id: null,
